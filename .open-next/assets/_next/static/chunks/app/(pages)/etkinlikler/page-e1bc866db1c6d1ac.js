(self.webpackChunk_N_E = self.webpackChunk_N_E || []).push([
  [175],
  {
    9870: (e, t, a) => {
      Promise.resolve().then(a.bind(a, 1013));
    },
    1013: (e, t, a) => {
      "use strict";
      (a.r(t), a.d(t, { default: () => E }));
      var n = a(5155),
        l = a(2115),
        r = a(3312),
        s = a(2056),
        c = a(2288);
      let o = new Map([
          [
            "bold",
            l.createElement(
              l.Fragment,
              null,
              l.createElement("path", {
                d: "M128,60a44,44,0,1,0,44,44A44.05,44.05,0,0,0,128,60Zm0,64a20,20,0,1,1,20-20A20,20,0,0,1,128,124Zm0-112a92.1,92.1,0,0,0-92,92c0,77.36,81.64,135.4,85.12,137.83a12,12,0,0,0,13.76,0,259,259,0,0,0,42.18-39C205.15,170.57,220,136.37,220,104A92.1,92.1,0,0,0,128,12Zm31.3,174.71A249.35,249.35,0,0,1,128,216.89a249.35,249.35,0,0,1-31.3-30.18C80,167.37,60,137.31,60,104a68,68,0,0,1,136,0C196,137.31,176,167.37,159.3,186.71Z",
              }),
            ),
          ],
          [
            "duotone",
            l.createElement(
              l.Fragment,
              null,
              l.createElement("path", {
                d: "M128,24a80,80,0,0,0-80,80c0,72,80,128,80,128s80-56,80-128A80,80,0,0,0,128,24Zm0,112a32,32,0,1,1,32-32A32,32,0,0,1,128,136Z",
                opacity: "0.2",
              }),
              l.createElement("path", {
                d: "M128,64a40,40,0,1,0,40,40A40,40,0,0,0,128,64Zm0,64a24,24,0,1,1,24-24A24,24,0,0,1,128,128Zm0-112a88.1,88.1,0,0,0-88,88c0,31.4,14.51,64.68,42,96.25a254.19,254.19,0,0,0,41.45,38.3,8,8,0,0,0,9.18,0A254.19,254.19,0,0,0,174,200.25c27.45-31.57,42-64.85,42-96.25A88.1,88.1,0,0,0,128,16Zm0,206c-16.53-13-72-60.75-72-118a72,72,0,0,1,144,0C200,161.23,144.53,209,128,222Z",
              }),
            ),
          ],
          [
            "fill",
            l.createElement(
              l.Fragment,
              null,
              l.createElement("path", {
                d: "M128,16a88.1,88.1,0,0,0-88,88c0,75.3,80,132.17,83.41,134.55a8,8,0,0,0,9.18,0C136,236.17,216,179.3,216,104A88.1,88.1,0,0,0,128,16Zm0,56a32,32,0,1,1-32,32A32,32,0,0,1,128,72Z",
              }),
            ),
          ],
          [
            "light",
            l.createElement(
              l.Fragment,
              null,
              l.createElement("path", {
                d: "M128,66a38,38,0,1,0,38,38A38,38,0,0,0,128,66Zm0,64a26,26,0,1,1,26-26A26,26,0,0,1,128,130Zm0-112a86.1,86.1,0,0,0-86,86c0,30.91,14.34,63.74,41.47,94.94a252.32,252.32,0,0,0,41.09,38,6,6,0,0,0,6.88,0,252.32,252.32,0,0,0,41.09-38c27.13-31.2,41.47-64,41.47-94.94A86.1,86.1,0,0,0,128,18Zm0,206.51C113,212.93,54,163.62,54,104a74,74,0,0,1,148,0C202,163.62,143,212.93,128,224.51Z",
              }),
            ),
          ],
          [
            "regular",
            l.createElement(
              l.Fragment,
              null,
              l.createElement("path", {
                d: "M128,64a40,40,0,1,0,40,40A40,40,0,0,0,128,64Zm0,64a24,24,0,1,1,24-24A24,24,0,0,1,128,128Zm0-112a88.1,88.1,0,0,0-88,88c0,31.4,14.51,64.68,42,96.25a254.19,254.19,0,0,0,41.45,38.3,8,8,0,0,0,9.18,0A254.19,254.19,0,0,0,174,200.25c27.45-31.57,42-64.85,42-96.25A88.1,88.1,0,0,0,128,16Zm0,206c-16.53-13-72-60.75-72-118a72,72,0,0,1,144,0C200,161.23,144.53,209,128,222Z",
              }),
            ),
          ],
          [
            "thin",
            l.createElement(
              l.Fragment,
              null,
              l.createElement("path", {
                d: "M128,68a36,36,0,1,0,36,36A36,36,0,0,0,128,68Zm0,64a28,28,0,1,1,28-28A28,28,0,0,1,128,132Zm0-112a84.09,84.09,0,0,0-84,84c0,30.42,14.17,62.79,41,93.62a250,250,0,0,0,40.73,37.66,4,4,0,0,0,4.58,0A250,250,0,0,0,171,197.62c26.81-30.83,41-63.2,41-93.62A84.09,84.09,0,0,0,128,20Zm37.1,172.23A254.62,254.62,0,0,1,128,227a254.62,254.62,0,0,1-37.1-34.81C73.15,171.8,52,139.9,52,104a76,76,0,0,1,152,0C204,139.9,182.85,171.8,165.1,192.23Z",
              }),
            ),
          ],
        ]),
        i = l.forwardRef((e, t) =>
          l.createElement(c.A, { ref: t, ...e, weights: o }),
        );
      i.displayName = "MapPinIcon";
      let d = new Map([
          [
            "bold",
            l.createElement(
              l.Fragment,
              null,
              l.createElement("path", {
                d: "M184.49,136.49l-80,80a12,12,0,0,1-17-17L159,128,87.51,56.49a12,12,0,1,1,17-17l80,80A12,12,0,0,1,184.49,136.49Z",
              }),
            ),
          ],
          [
            "duotone",
            l.createElement(
              l.Fragment,
              null,
              l.createElement("path", {
                d: "M176,128,96,208V48Z",
                opacity: "0.2",
              }),
              l.createElement("path", {
                d: "M181.66,122.34l-80-80A8,8,0,0,0,88,48V208a8,8,0,0,0,13.66,5.66l80-80A8,8,0,0,0,181.66,122.34ZM104,188.69V67.31L164.69,128Z",
              }),
            ),
          ],
          [
            "fill",
            l.createElement(
              l.Fragment,
              null,
              l.createElement("path", {
                d: "M181.66,133.66l-80,80A8,8,0,0,1,88,208V48a8,8,0,0,1,13.66-5.66l80,80A8,8,0,0,1,181.66,133.66Z",
              }),
            ),
          ],
          [
            "light",
            l.createElement(
              l.Fragment,
              null,
              l.createElement("path", {
                d: "M180.24,132.24l-80,80a6,6,0,0,1-8.48-8.48L167.51,128,91.76,52.24a6,6,0,0,1,8.48-8.48l80,80A6,6,0,0,1,180.24,132.24Z",
              }),
            ),
          ],
          [
            "regular",
            l.createElement(
              l.Fragment,
              null,
              l.createElement("path", {
                d: "M181.66,133.66l-80,80a8,8,0,0,1-11.32-11.32L164.69,128,90.34,53.66a8,8,0,0,1,11.32-11.32l80,80A8,8,0,0,1,181.66,133.66Z",
              }),
            ),
          ],
          [
            "thin",
            l.createElement(
              l.Fragment,
              null,
              l.createElement("path", {
                d: "M178.83,130.83l-80,80a4,4,0,0,1-5.66-5.66L170.34,128,93.17,50.83a4,4,0,0,1,5.66-5.66l80,80A4,4,0,0,1,178.83,130.83Z",
              }),
            ),
          ],
        ]),
        m = l.forwardRef((e, t) =>
          l.createElement(c.A, { ref: t, ...e, weights: d }),
        );
      m.displayName = "CaretRightIcon";
      let u = new Map([
          [
            "bold",
            l.createElement(
              l.Fragment,
              null,
              l.createElement("path", {
                d: "M224.49,136.49l-72,72a12,12,0,0,1-17-17L187,140H40a12,12,0,0,1,0-24H187L135.51,64.48a12,12,0,0,1,17-17l72,72A12,12,0,0,1,224.49,136.49Z",
              }),
            ),
          ],
          [
            "duotone",
            l.createElement(
              l.Fragment,
              null,
              l.createElement("path", {
                d: "M216,128l-72,72V56Z",
                opacity: "0.2",
              }),
              l.createElement("path", {
                d: "M221.66,122.34l-72-72A8,8,0,0,0,136,56v64H40a8,8,0,0,0,0,16h96v64a8,8,0,0,0,13.66,5.66l72-72A8,8,0,0,0,221.66,122.34ZM152,180.69V75.31L204.69,128Z",
              }),
            ),
          ],
          [
            "fill",
            l.createElement(
              l.Fragment,
              null,
              l.createElement("path", {
                d: "M221.66,133.66l-72,72A8,8,0,0,1,136,200V136H40a8,8,0,0,1,0-16h96V56a8,8,0,0,1,13.66-5.66l72,72A8,8,0,0,1,221.66,133.66Z",
              }),
            ),
          ],
          [
            "light",
            l.createElement(
              l.Fragment,
              null,
              l.createElement("path", {
                d: "M220.24,132.24l-72,72a6,6,0,0,1-8.48-8.48L201.51,134H40a6,6,0,0,1,0-12H201.51L139.76,60.24a6,6,0,0,1,8.48-8.48l72,72A6,6,0,0,1,220.24,132.24Z",
              }),
            ),
          ],
          [
            "regular",
            l.createElement(
              l.Fragment,
              null,
              l.createElement("path", {
                d: "M221.66,133.66l-72,72a8,8,0,0,1-11.32-11.32L196.69,136H40a8,8,0,0,1,0-16H196.69L138.34,61.66a8,8,0,0,1,11.32-11.32l72,72A8,8,0,0,1,221.66,133.66Z",
              }),
            ),
          ],
          [
            "thin",
            l.createElement(
              l.Fragment,
              null,
              l.createElement("path", {
                d: "M218.83,130.83l-72,72a4,4,0,0,1-5.66-5.66L206.34,132H40a4,4,0,0,1,0-8H206.34L141.17,58.83a4,4,0,0,1,5.66-5.66l72,72A4,4,0,0,1,218.83,130.83Z",
              }),
            ),
          ],
        ]),
        h = l.forwardRef((e, t) =>
          l.createElement(c.A, { ref: t, ...e, weights: u }),
        );
      h.displayName = "ArrowRightIcon";
      let p = (0, a(4057).A)("LoaderCircle", [
        ["path", { d: "M21 12a9 9 0 1 1-6.219-8.56", key: "13zald" }],
      ]);
      var g = a(9645),
        x = a(6046),
        v = a(4130),
        f = a(7847),
        b = a(5565);
      function w(e) {
        var t, a;
        let {
            event: c,
            availableYears: o,
            selectedYear: d,
            isLatestEvent: u = !1,
            allEvents: w = [],
          } = e,
          y = (0, x.useRouter)(),
          [A, E] = (0, l.useState)(!1),
          V = c.name
            .replace(/[-\s]\d{4}$/, "")
            .toLowerCase()
            .split(/[-\s]+/)
            .filter(Boolean)
            .map((e) => "#".concat(e)),
          j = c.name
            .replace(/-\d{4}$/, "")
            .split("-")
            .map((e) => e.charAt(0).toUpperCase() + e.slice(1))
            .join(" "),
          Z =
            (null === (t = c.name.match(/-(\d{4})$/)) || void 0 === t
              ? void 0
              : t[1]) || "";
        return (0, n.jsx)("div", {
          className:
            "bg-color-primary rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 border ".concat(
              u
                ? "border-color-secondary border-2 ring-2 ring-color-secondary/20"
                : "border-gray-100",
            ),
          children: (0, n.jsxs)("div", {
            className:
              "flex flex-col md:flex-row items-stretch p-8 gap-8 md:gap-12",
            children: [
              (0, n.jsxs)("div", {
                className:
                  "flex-1 min-w-0 flex flex-col justify-center max-w-xl lg:max-w-lg xl:max-w-xl",
                children: [
                  (0, n.jsxs)("div", {
                    className: "block md:hidden mb-6",
                    children: [
                      u &&
                        (0, n.jsx)("div", {
                          className: "mb-1",
                          children: (0, n.jsx)("span", {
                            className:
                              "bg-color-secondary text-white px-3 py-1 rounded-full text-xs font-semibold",
                            children: "YAKLAŞIYOR",
                          }),
                        }),
                      (0, n.jsx)("div", {
                        className: "flex gap-2 items-center",
                        children: V.map((e, t) =>
                          (0, n.jsx)(
                            "span",
                            {
                              className: "text-sm text-color-text font-medium",
                              children: e,
                            },
                            t,
                          ),
                        ),
                      }),
                    ],
                  }),
                  (0, n.jsxs)("div", {
                    className: "hidden md:flex gap-2 mb-6 items-center",
                    children: [
                      V.map((e, t) =>
                        (0, n.jsx)(
                          "span",
                          {
                            className: "text-sm text-color-text font-medium",
                            children: e,
                          },
                          t,
                        ),
                      ),
                      u &&
                        (0, n.jsx)("span", {
                          className:
                            "bg-color-secondary text-white px-3 py-1 rounded-full text-xs font-semibold ml-2",
                          children: "YAKLAŞIYOR",
                        }),
                    ],
                  }),
                  (0, n.jsx)("div", {
                    className: "block md:hidden mb-4",
                    children: (() => {
                      let e = c.name
                          .replace(/\s+\d{4}$|-\d{4}$/, "")
                          .toLowerCase()
                          .replace(/ /g, "-"),
                        t = Math.max(...o.map((e) => parseInt(e))),
                        a = ""
                          .concat(f.vw, "/")
                          .concat(e, "-")
                          .concat(t, "/banner.webp");
                      return (0, n.jsx)(b.default, {
                        src: a,
                        alt: j,
                        width: 480,
                        height: 270,
                        className:
                          "rounded-2xl w-full h-auto object-cover shadow-md",
                        style: { background: "#f3f0fa" },
                        onError: (e) => {
                          e.currentTarget.style.display = "none";
                        },
                        priority: u,
                      });
                    })(),
                  }),
                  (0, n.jsxs)("div", {
                    className: "space-y-6",
                    children: [
                      (0, n.jsxs)("div", {
                        children: [
                          (0, n.jsxs)("h2", {
                            className:
                              "text-3xl lg:text-4xl font-bold text-color-text mb-2",
                            children: [j, " ", Z],
                          }),
                          (0, n.jsxs)("div", {
                            className:
                              "flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 text-color-text mb-4",
                            children: [
                              (0, n.jsxs)("div", {
                                className:
                                  "flex items-center gap-2 justify-start",
                                children: [
                                  (0, n.jsx)(s.V, { weight: "fill", size: 18 }),
                                  (0, n.jsx)("span", {
                                    className: "font-medium",
                                    children: (0, v.po)(c.date),
                                  }),
                                ],
                              }),
                              "" !== c.location.name &&
                                (0, n.jsxs)("div", {
                                  className:
                                    "flex items-center gap-2 justify-start",
                                  children: [
                                    (0, n.jsx)(i, { weight: "fill", size: 18 }),
                                    (0, n.jsx)("span", {
                                      children: c.location.name,
                                    }),
                                  ],
                                }),
                            ],
                          }),
                          (0, n.jsx)("p", {
                            className: "text-color-text leading-relaxed",
                            children: c.heroDescription,
                          }),
                        ],
                      }),
                      (0, n.jsx)("div", {
                        className: "flex items-center gap-1 mt-2 select-none",
                        children: o.map((e) => {
                          let t = c.name.replace(/\s+\d{4}$|-\d{4}$/, ""),
                            a = "".concat(t, " ").concat(e),
                            l = (0, g.Y)(a),
                            r =
                              e ===
                              Math.max(...o.map((e) => parseInt(e))).toString(),
                            s = (0, v.Cv)(w, t, e) || c,
                            i = new Date(s.date) < new Date(),
                            m = !1 === s.navigable && !i,
                            h = e === d;
                          return (0, n.jsx)(
                            "button",
                            {
                              onClick: () => {
                                if (!m) {
                                  if (
                                    !1 === s.navigable &&
                                    i &&
                                    s.registerLink
                                  ) {
                                    window.open(s.registerLink, "_blank");
                                    return;
                                  }
                                  u && r
                                    ? y.push("/")
                                    : y.push("/etkinlikler/".concat(l));
                                }
                              },
                              disabled: m,
                              className:
                                "transition-all duration-150 text-base md:text-lg font-semibold pr-2 py-1 rounded-md border-none outline-none focus:outline-none\n                      "
                                  .concat(
                                    h
                                      ? "font-extrabold text-[#4d002f] bg-transparent opacity-100"
                                      : "font-semibold text-color-text opacity-40 hover:opacity-80 hover:bg-color-accent/10 cursor-pointer",
                                    "\n                      ",
                                  )
                                  .concat(
                                    m ? "opacity-30 cursor-not-allowed" : "",
                                    "\n                    ",
                                  ),
                              style: {
                                WebkitTextStroke: h ? "0.3px #4d002f" : void 0,
                              },
                              tabIndex: m ? -1 : 0,
                              type: "button",
                              children: e,
                            },
                            e,
                          );
                        }),
                      }),
                      (0, n.jsxs)("div", {
                        className: "flex flex-col sm:flex-row gap-3",
                        children: [
                          (0, n.jsx)(r.$, {
                            onClick:
                              !1 !== c.navigable ||
                              new Date(c.date) < new Date()
                                ? () => {
                                    E(!0);
                                    let e = new Date(c.date),
                                      t = new Date();
                                    if (
                                      !1 === c.navigable &&
                                      e < t &&
                                      c.registerLink
                                    ) {
                                      (window.open(c.registerLink, "_blank"),
                                        E(!1));
                                      return;
                                    }
                                    if (u) y.push("/");
                                    else {
                                      let e = "/etkinlikler/".concat(
                                        (0, g.Y)(c.name),
                                      );
                                      y.push(e);
                                    }
                                  }
                                : void 0,
                            disabled:
                              A ||
                              (!1 === c.navigable &&
                                !(new Date(c.date) < new Date())),
                            className:
                              "group bg-color-secondary text-white hover:bg-gray-600 hover:shadow-md active:bg-color-accent transition-all duration-300 px-6 py-3 rounded-lg ".concat(
                                !1 !== c.navigable ||
                                  new Date(c.date) < new Date()
                                  ? ""
                                  : "opacity-50 cursor-not-allowed",
                              ),
                            children: A
                              ? (0, n.jsxs)("span", {
                                  className: "flex items-center gap-2",
                                  children: [
                                    (0, n.jsx)(p, {
                                      className: "animate-spin",
                                      size: 16,
                                    }),
                                    "Y\xfckleniyor...",
                                  ],
                                })
                              : (0, n.jsx)(n.Fragment, {
                                  children:
                                    !1 !== c.navigable ||
                                    new Date(c.date) < new Date()
                                      ? (0, n.jsxs)("span", {
                                          className: "flex items-center",
                                          children: [
                                            (0, n.jsx)("span", {
                                              children: "Daha Fazla",
                                            }),
                                            (0, n.jsxs)("span", {
                                              className:
                                                "relative ml-2 inline-flex items-center justify-center w-5 h-5 align-middle",
                                              children: [
                                                (0, n.jsx)(m, {
                                                  className:
                                                    "absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 transition-all duration-300 opacity-100 group-hover:opacity-0 group-hover:translate-x-2",
                                                  weight: "bold",
                                                  size: 16,
                                                }),
                                                (0, n.jsx)(h, {
                                                  className:
                                                    "absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 transition-all duration-300 opacity-0 group-hover:opacity-100 group-hover:translate-x-0",
                                                  weight: "bold",
                                                  size: 16,
                                                }),
                                              ],
                                            }),
                                          ],
                                        })
                                      : (0, n.jsx)("span", {
                                          className: "flex items-center",
                                          children: "\xc7ok Yakında!",
                                        }),
                                }),
                          }),
                          (0, n.jsxs)(n.Fragment, {
                            children: [
                              c.tickets &&
                                c.tickets.length > 0 &&
                                (null === (a = c.tickets[0]) || void 0 === a
                                  ? void 0
                                  : a.link) &&
                                (0, n.jsx)(r.$, {
                                  onClick: () =>
                                    window.open(c.tickets[0].link, "_blank"),
                                  className:
                                    "bg-black text-white hover:bg-gray-800 hover:shadow-md active:bg-gray-900 transition-all duration-300 px-6 py-3 rounded-lg",
                                  children: (0, n.jsx)("span", {
                                    className: "flex items-center",
                                    children: "Biletler",
                                  }),
                                }),
                              (0, n.jsx)(r.$, {
                                onClick: () =>
                                  window.open(
                                    "mailto:developermultigroup@gmail.com",
                                    "_blank",
                                  ),
                                className:
                                  "bg-black text-white hover:bg-gray-800 hover:shadow-md active:bg-gray-900 transition-all duration-300 px-6 py-3 rounded-lg",
                                children: (0, n.jsx)("span", {
                                  className: "flex items-center",
                                  children: "Sponsor Ol",
                                }),
                              }),
                            ],
                          }),
                        ],
                      }),
                    ],
                  }),
                ],
              }),
              (0, n.jsx)("div", {
                className: "hidden md:flex flex-1 items-center justify-center",
                children: (() => {
                  let e = c.name
                      .replace(/\s+\d{4}$|-\d{4}$/, "")
                      .toLowerCase()
                      .replace(/ /g, "-"),
                    t = Math.max(...o.map((e) => parseInt(e))),
                    a = (0, f.J_)(
                      "".concat(e, "-").concat(t),
                      "banners",
                      "".concat(e, "-").concat(t, ".webp"),
                    );
                  return (0, n.jsx)("div", {
                    className: "w-full h-full flex items-center justify-center",
                    children: (0, n.jsx)(b.default, {
                      src: a,
                      alt: j,
                      width: 480,
                      height: 270,
                      className:
                        "rounded-2xl w-full h-auto max-h-[400px] object-cover shadow-md",
                      style: { background: "#f3f0fa" },
                      onError: (e) => {
                        e.currentTarget.style.display = "none";
                      },
                      priority: u,
                    }),
                  });
                })(),
              }),
            ],
          }),
        });
      }
      var y = a(54),
        A = a(3388);
      function E() {
        let [e, t] = (0, l.useState)(null),
          [a, r] = (0, l.useState)(null),
          [s, c] = (0, l.useState)([]),
          { setCurrentEvent: o } = (0, y.j)();
        if (
          ((0, l.useEffect)(() => {
            fetch("/api/events")
              .then((e) => e.json())
              .then((e) => {
                if ((t(e), 0 === e.length)) return;
                let a = (0, v.WU)(e);
                (r(a), o(a));
              })
              .catch(() => t([]));
          }, [o]),
          (0, l.useEffect)(() => {
            if (!a || !e) return;
            let t = new Map();
            e.forEach((e) => {
              let a = e.name.match(/^(.+?)\s+(\d{4})$|^(.+)-(\d{4})$/);
              if (a) {
                let n = a[1] || a[3];
                (a[2] || a[4], t.has(n) || t.set(n, []), t.get(n).push(e));
              }
            });
            let n = [];
            (t.forEach((e, t) => {
              e.sort((e, t) => {
                var a, n;
                let l =
                  (null === (a = e.name.match(/\s+(\d{4})$|-(\d{4})$/)) ||
                  void 0 === a
                    ? void 0
                    : a[1]) || "0";
                return (
                  Number.parseInt(
                    (null === (n = t.name.match(/\s+(\d{4})$|-(\d{4})$/)) ||
                    void 0 === n
                      ? void 0
                      : n[1]) || "0",
                  ) - Number.parseInt(l)
                );
              });
              let a = e
                .map((e) => {
                  let t = e.name.match(/\s+(\d{4})$|-(\d{4})$/);
                  return t ? t[1] : "";
                })
                .filter(Boolean);
              n.push({
                baseEvent: e[0],
                availableYears: a,
                selectedYear: a[0],
              });
            }),
              n.sort(
                (e, t) =>
                  new Date(t.baseEvent.date).getTime() -
                  new Date(e.baseEvent.date).getTime(),
              ));
            let l = n.findIndex(
              (e) =>
                e.baseEvent.id === a.id ||
                n.some((t) =>
                  t.availableYears.some(
                    (t) =>
                      e.baseEvent.name.replace(/\s+\d{4}$|-\d{4}$/, "") ===
                      a.name.replace(/\s+\d{4}$|-\d{4}$/, ""),
                  ),
                ),
            );
            if (l > 0) {
              let e = n.splice(l, 1)[0];
              n.unshift(e);
            }
            c(n);
          }, [a, e]),
          !e || !a)
        )
          return (0, n.jsx)(A.default, {});
        let i = s.map((e) => ({
            event: e.baseEvent,
            availableYears: e.availableYears,
            selectedYear: e.selectedYear,
            isLatestEvent: e.baseEvent.id === a.id,
          })),
          d = new Date(),
          m = i
            .filter((e) => {
              let { event: t } = e;
              return new Date(t.date) > d;
            })
            .sort(
              (e, t) =>
                new Date(e.event.date).getTime() -
                new Date(t.event.date).getTime(),
            ),
          u = i
            .filter((e) => {
              let { event: t } = e;
              return new Date(t.date) <= d;
            })
            .sort(
              (e, t) =>
                new Date(t.event.date).getTime() -
                new Date(e.event.date).getTime(),
            );
        return (
          m.length > 0 &&
            (m.forEach((e, t) => (e.isLatestEvent = !1)),
            (m[0].isLatestEvent = !0)),
          (0, n.jsx)("div", {
            className: "min-h-screen bg-color-background",
            children: (0, n.jsxs)("div", {
              className: "pt-[20vh] w-5/6 2xl:w-2/3 mx-auto space-y-8",
              children: [
                m.length > 0 &&
                  (0, n.jsx)("div", {
                    className: "space-y-8",
                    children: m.map((t) =>
                      (0, n.jsx)(
                        w,
                        { ...t, allEvents: e },
                        "".concat(t.event.id, "-").concat(t.selectedYear),
                      ),
                    ),
                  }),
                m.length > 0 &&
                  u.length > 0 &&
                  (0, n.jsx)("hr", {
                    className: "my-8 border-t border-gray-300",
                  }),
                u.length > 0 &&
                  (0, n.jsx)("div", {
                    className: "space-y-8 ",
                    children: u.map((t) =>
                      (0, n.jsx)(
                        w,
                        { ...t, allEvents: e },
                        "".concat(t.event.id, "-").concat(t.selectedYear),
                      ),
                    ),
                  }),
                0 === m.length &&
                  0 === u.length &&
                  (0, n.jsx)("div", {
                    className:
                      "flex items-center justify-center min-h-[50vh] text-lg text-gray-600",
                    children: "Ge\xe7miş etkinlik bulunmamaktadır.",
                  }),
              ],
            }),
          })
        );
      }
    },
    3388: (e, t, a) => {
      "use strict";
      (a.r(t), a.d(t, { default: () => r }));
      var n = a(5155),
        l = a(7847);
      function r() {
        return (0, n.jsxs)("div", {
          className:
            "min-h-screen flex flex-col items-center justify-center bg-black",
          children: [
            (0, n.jsx)("img", {
              src: (0, l.b8)("/images/logo/logo-small-white.webp"),
              alt: "Logo",
              className: "mb-8 w-16",
            }),
            (0, n.jsx)("div", {
              className:
                "w-48 h-px bg-gray-600 rounded-full overflow-hidden relative",
              children: (0, n.jsx)("div", {
                className: "h-full bg-white animate-loading",
              }),
            }),
          ],
        });
      }
    },
    3312: (e, t, a) => {
      "use strict";
      a.d(t, { $: () => i });
      var n = a(5155),
        l = a(2115),
        r = a(2317),
        s = a(1027),
        c = a(1567);
      let o = (0, s.F)(
          "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
          {
            variants: {
              variant: {
                default:
                  "bg-primary text-primary-foreground shadow hover:bg-primary/90",
                destructive:
                  "bg-destructive text-destructive-foreground shadow-sm hover:bg-destructive/90",
                outline:
                  "border border-input bg-background shadow-sm hover:bg-accent hover:text-accent-foreground",
                secondary:
                  "bg-secondary text-secondary-foreground shadow-sm hover:bg-secondary/80",
                ghost: "hover:bg-accent hover:text-accent-foreground",
                link: "text-primary underline-offset-4 hover:underline",
              },
              size: {
                default: "h-9 px-4 py-2",
                sm: "h-8 rounded-md px-3 text-xs",
                lg: "h-10 rounded-md px-8",
                icon: "h-9 w-9",
              },
            },
            defaultVariants: { variant: "default", size: "default" },
          },
        ),
        i = l.forwardRef((e, t) => {
          let { className: a, variant: l, size: s, asChild: i = !1, ...d } = e,
            m = i ? r.DX : "button";
          return (0, n.jsx)(m, {
            className: (0, c.cn)(o({ variant: l, size: s, className: a })),
            ref: t,
            ...d,
          });
        });
      i.displayName = "Button";
    },
    54: (e, t, a) => {
      "use strict";
      a.d(t, { D: () => o, j: () => i });
      var n = a(5155),
        l = a(2115),
        r = a(6046);
      let s = (0, l.createContext)({
          currentEvent: null,
          setCurrentEvent: () => {},
        }),
        c = {
          primary: "348 83% 47%",
          secondary: "348 83% 47%",
          accent: "348 83% 47%",
          background: "0 0% 6%",
          text: "0 0% 100%",
        },
        o = (e) => {
          var t;
          let { children: a, events: o = [] } = e,
            [i, d] = (0, l.useState)(
              null !== (t = o[0]) && void 0 !== t ? t : null,
            ),
            m = (0, r.usePathname)();
          return (
            l.useEffect(() => {
              o.length > 0 && !i && d(o[0]);
            }, [o, i]),
            l.useEffect(() => {
              let e =
                "/404" === m || "/not-found" === m
                  ? c
                  : null == i
                    ? void 0
                    : i.colorPalette;
              e &&
                (document.documentElement.style.setProperty(
                  "--color-primary",
                  e.primary,
                ),
                document.documentElement.style.setProperty(
                  "--color-secondary",
                  e.secondary,
                ),
                document.documentElement.style.setProperty(
                  "--color-accent",
                  e.accent,
                ),
                document.documentElement.style.setProperty(
                  "--color-background",
                  e.background,
                ),
                document.documentElement.style.setProperty(
                  "--color-text",
                  e.text,
                ));
            }, [i, m]),
            (0, n.jsx)(s.Provider, {
              value: { currentEvent: i, setCurrentEvent: d },
              children: a,
            })
          );
        },
        i = () => (0, l.useContext)(s);
    },
    4130: (e, t, a) => {
      "use strict";
      function n(e) {
        return new Date(e).toLocaleDateString("tr-TR", {
          day: "numeric",
          month: "long",
          year: "numeric",
        });
      }
      function l(e) {
        return e
          .slice()
          .sort(
            (e, t) => new Date(e.date).getTime() - new Date(t.date).getTime(),
          );
      }
      function r(e) {
        let t = l(e);
        return t[t.length - 1];
      }
      function s(e) {
        let t = (function (e) {
          let t = new Date();
          return (
            e
              .filter((e) => new Date(e.date) > t)
              .sort(
                (e, t) =>
                  new Date(e.date).getTime() - new Date(t.date).getTime(),
              )[0] || null
          );
        })(e);
        return t
          ? t.registerLink
          : "https://togather.lodos.io/communities/multiacademy-94761667282726876508";
      }
      function c(e) {
        let t = l(e);
        return t[t.length - 2];
      }
      function o(e) {
        let t = e.filter((e) => !1 !== e.navigable);
        return 0 === t.length
          ? null
          : t.sort(
              (e, t) => new Date(t.date).getTime() - new Date(e.date).getTime(),
            )[0];
      }
      function i(e, t, a) {
        let n = t
          .trim()
          .toLowerCase()
          .replace(/[-\s]+/g, "-");
        return (
          e.find((e) => {
            var t;
            let l = e.name.trim().toLowerCase(),
              r = l.replace(/[-\s]?\d{4}$/, "").replace(/[-\s]+/g, "-"),
              s =
                null === (t = l.match(/(\d{4})$/)) || void 0 === t
                  ? void 0
                  : t[1];
            return r === n && s === a;
          }) || null
        );
      }
      a.d(t, {
        Cv: () => i,
        FX: () => o,
        Go: () => s,
        SG: () => c,
        WU: () => r,
        po: () => n,
      });
    },
    7847: (e, t, a) => {
      "use strict";
      a.d(t, { AL: () => s, J_: () => r, b8: () => l, vw: () => n });
      let n = "https://academy-assets.devmultigroup.com";
      function l(e) {
        let t = e.replace(/^\/images\//, "");
        return t.startsWith("logo/")
          ? "".concat(n, "/shared/").concat(t)
          : "".concat(n, "/").concat(t);
      }
      function r(e, t, a) {
        return "".concat(n, "/").concat(e, "/").concat(t, "/").concat(a);
      }
      function s(e, t) {
        let a = t.split("/").pop() || t;
        return ""
          .concat(n, "/")
          .concat(e, "/")
          .concat(a.startsWith("organizers") ? "" : "organizers/")
          .concat(a);
      }
    },
    9645: (e, t, a) => {
      "use strict";
      function n(e) {
        let t = {
          ç: "c",
          ğ: "g",
          ı: "i",
          ö: "o",
          ş: "s",
          ü: "u",
          Ç: "C",
          Ğ: "G",
          İ: "I",
          Ö: "O",
          Ş: "S",
          Ü: "U",
        };
        return e
          .split("")
          .map((e) => t[e] || e)
          .join("")
          .toLowerCase()
          .replace(/[^a-z0-9-]+/g, "-")
          .replace(/^-+|-+$/g, "")
          .replace(/--+/g, "-");
      }
      a.d(t, { Y: () => n });
    },
    1567: (e, t, a) => {
      "use strict";
      a.d(t, { cn: () => r });
      var n = a(3463),
        l = a(9795);
      function r() {
        for (var e = arguments.length, t = Array(e), a = 0; a < e; a++)
          t[a] = arguments[a];
        return (0, l.QP)((0, n.$)(t));
      }
    },
    2056: (e, t, a) => {
      "use strict";
      a.d(t, { V: () => c });
      var n = a(2115),
        l = a(2288);
      let r = new Map([
          [
            "bold",
            n.createElement(
              n.Fragment,
              null,
              n.createElement("path", {
                d: "M208,28H188V24a12,12,0,0,0-24,0v4H92V24a12,12,0,0,0-24,0v4H48A20,20,0,0,0,28,48V208a20,20,0,0,0,20,20H208a20,20,0,0,0,20-20V48A20,20,0,0,0,208,28ZM68,52a12,12,0,0,0,24,0h72a12,12,0,0,0,24,0h16V76H52V52ZM52,204V100H204V204Zm60-80v56a12,12,0,0,1-24,0V143.32a12,12,0,0,1-9.37-22l16-8A12,12,0,0,1,112,124Zm61.49,33.88L163.9,168H168a12,12,0,0,1,0,24H136a12,12,0,0,1-8.71-20.25L155.45,142a4,4,0,0,0,.55-2,4,4,0,0,0-7.47-2,12,12,0,0,1-20.78-12A28,28,0,0,1,180,140a27.77,27.77,0,0,1-5.64,16.86A10.63,10.63,0,0,1,173.49,157.88Z",
              }),
            ),
          ],
          [
            "duotone",
            n.createElement(
              n.Fragment,
              null,
              n.createElement("path", {
                d: "M216,48V88H40V48a8,8,0,0,1,8-8H208A8,8,0,0,1,216,48Z",
                opacity: "0.2",
              }),
              n.createElement("path", {
                d: "M208,32H184V24a8,8,0,0,0-16,0v8H88V24a8,8,0,0,0-16,0v8H48A16,16,0,0,0,32,48V208a16,16,0,0,0,16,16H208a16,16,0,0,0,16-16V48A16,16,0,0,0,208,32ZM72,48v8a8,8,0,0,0,16,0V48h80v8a8,8,0,0,0,16,0V48h24V80H48V48ZM208,208H48V96H208V208Zm-96-88v64a8,8,0,0,1-16,0V132.94l-4.42,2.22a8,8,0,0,1-7.16-14.32l16-8A8,8,0,0,1,112,120Zm59.16,30.45L152,176h16a8,8,0,0,1,0,16H136a8,8,0,0,1-6.4-12.8l28.78-38.37A8,8,0,1,0,145.07,132a8,8,0,1,1-13.85-8A24,24,0,0,1,176,136,23.76,23.76,0,0,1,171.16,150.45Z",
              }),
            ),
          ],
          [
            "fill",
            n.createElement(
              n.Fragment,
              null,
              n.createElement("path", {
                d: "M208,32H184V24a8,8,0,0,0-16,0v8H88V24a8,8,0,0,0-16,0v8H48A16,16,0,0,0,32,48V208a16,16,0,0,0,16,16H208a16,16,0,0,0,16-16V48A16,16,0,0,0,208,32ZM112,184a8,8,0,0,1-16,0V132.94l-4.42,2.22a8,8,0,0,1-7.16-14.32l16-8A8,8,0,0,1,112,120Zm56-8a8,8,0,0,1,0,16H136a8,8,0,0,1-6.4-12.8l28.78-38.37A8,8,0,1,0,145.07,132a8,8,0,1,1-13.85-8A24,24,0,0,1,176,136a23.76,23.76,0,0,1-4.84,14.45L152,176ZM48,80V48H72v8a8,8,0,0,0,16,0V48h80v8a8,8,0,0,0,16,0V48h24V80Z",
              }),
            ),
          ],
          [
            "light",
            n.createElement(
              n.Fragment,
              null,
              n.createElement("path", {
                d: "M208,34H182V24a6,6,0,0,0-12,0V34H86V24a6,6,0,0,0-12,0V34H48A14,14,0,0,0,34,48V208a14,14,0,0,0,14,14H208a14,14,0,0,0,14-14V48A14,14,0,0,0,208,34ZM48,46H74V56a6,6,0,0,0,12,0V46h84V56a6,6,0,0,0,12,0V46h26a2,2,0,0,1,2,2V82H46V48A2,2,0,0,1,48,46ZM208,210H48a2,2,0,0,1-2-2V94H210V208A2,2,0,0,1,208,210Zm-98-90v64a6,6,0,0,1-12,0V129.71l-7.32,3.66a6,6,0,1,1-5.36-10.74l16-8A6,6,0,0,1,110,120Zm59.57,29.25L148,178h20a6,6,0,0,1,0,12H136a6,6,0,0,1-4.8-9.6L160,142a10,10,0,1,0-16.65-11A6,6,0,1,1,133,125a22,22,0,1,1,36.62,24.26Z",
              }),
            ),
          ],
          [
            "regular",
            n.createElement(
              n.Fragment,
              null,
              n.createElement("path", {
                d: "M208,32H184V24a8,8,0,0,0-16,0v8H88V24a8,8,0,0,0-16,0v8H48A16,16,0,0,0,32,48V208a16,16,0,0,0,16,16H208a16,16,0,0,0,16-16V48A16,16,0,0,0,208,32ZM72,48v8a8,8,0,0,0,16,0V48h80v8a8,8,0,0,0,16,0V48h24V80H48V48ZM208,208H48V96H208V208Zm-96-88v64a8,8,0,0,1-16,0V132.94l-4.42,2.22a8,8,0,0,1-7.16-14.32l16-8A8,8,0,0,1,112,120Zm59.16,30.45L152,176h16a8,8,0,0,1,0,16H136a8,8,0,0,1-6.4-12.8l28.78-38.37A8,8,0,1,0,145.07,132a8,8,0,1,1-13.85-8A24,24,0,0,1,176,136,23.76,23.76,0,0,1,171.16,150.45Z",
              }),
            ),
          ],
          [
            "thin",
            n.createElement(
              n.Fragment,
              null,
              n.createElement("path", {
                d: "M208,36H180V24a4,4,0,0,0-8,0V36H84V24a4,4,0,0,0-8,0V36H48A12,12,0,0,0,36,48V208a12,12,0,0,0,12,12H208a12,12,0,0,0,12-12V48A12,12,0,0,0,208,36ZM48,44H76V56a4,4,0,0,0,8,0V44h88V56a4,4,0,0,0,8,0V44h28a4,4,0,0,1,4,4V84H44V48A4,4,0,0,1,48,44ZM208,212H48a4,4,0,0,1-4-4V92H212V208A4,4,0,0,1,208,212ZM108,120v64a4,4,0,0,1-8,0V126.47l-10.21,5.11a4,4,0,0,1-3.58-7.16l16-8A4,4,0,0,1,108,120Zm60,28-24,32h24a4,4,0,0,1,0,8H136a4,4,0,0,1-3.2-6.4l28.78-38.37A11.88,11.88,0,0,0,164,136a12,12,0,0,0-22.4-6,4,4,0,0,1-6.92-4A20,20,0,0,1,172,136,19.79,19.79,0,0,1,168,148Z",
              }),
            ),
          ],
        ]),
        s = n.forwardRef((e, t) =>
          n.createElement(l.A, { ref: t, ...e, weights: r }),
        );
      s.displayName = "CalendarIcon";
      let c = s;
    },
  },
  (e) => {
    var t = (t) => e((e.s = t));
    (e.O(0, [346, 112, 441, 517, 358], () => t(9870)), (_N_E = e.O()));
  },
]);
